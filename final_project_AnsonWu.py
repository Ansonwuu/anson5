...
# Charts
st.success("Here are some charts to compare the numbers of pubs that are in hotels and not in hotels")
hotel_list = list(name_hotel.name.values)
...
not_hotel = df_pubs.loc[~df_pubs['name'].str.contains( "Hotel|Inn", case=False)]
not_hotel_list = list(not_hotel.name.values)
count1 = 0
for element1 in not_hotel_list:
    count1 += 1


a = ["Pubs in the hotel","Pubs not in the hotel"]
b = [count,count1]

# plot figures with multiple axes
...
d = ["Pubs not in hotel"]
num_in_hotel = [count]
num_not_hotel = [count1]
ax1.bar(c,num_in_hotel)


ax2.bar(a,b,color = "g")
ax3.pie(b, labels = a, autopct='%.2f%%')

ax4.bar(d,num_not_hotel,color = "black")
st.pyplot(fig)
